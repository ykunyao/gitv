...
try {
  // This file exists only right after `bun run compile` (which deletes it
  // again once the executable is built). In a normal checkout the import
  // fails and we fall back to building the frontend from web/ on disk.
...
let buildCache: { key: string; body: string } | null = null;
let buildSeq = 0;

/** Newest mtime across web/ + src/model.ts — a cheap freshness fingerprint. */
async function webSourceKey(): Promise<string> {
  let newest = 0;
  const walk = async (dir: string): Promise<void> => {
    for (const name of [...new Bun.Glob("*").scanSync({ cwd: dir, onlyFiles: true })]) {
      const st = await Bun.file(join(dir, name)).stat();
      if (st?.mtimeMs && st.mtimeMs > newest) newest = st.mtimeMs;
    }
  };
  await walk(WEB_ROOT);
  const model = await Bun.file(join(import.meta.dir, "model.ts")).stat();
  if (model?.mtimeMs && model.mtimeMs > newest) newest = model.mtimeMs;
  return String(newest);
}

async function buildFrontend(): Promise<Response> {
  const key = await webSourceKey();
  if (buildCache?.key === key) {
    return new Response(buildCache.body, {
      headers: { "content-type": "application/javascript; charset=utf-8", "cache-control": "no-store" },
    });
  }
  // Bun's in-process module cache doesn't notice on-disk edits, so build from
  // a fresh copy of the sources under a unique path every rebuild.
  const dir = join(import.meta.dir, "..", "tmp", `webbuild-${process.pid}-${++buildSeq}`);
  try {
    mkdirSync(join(dir, "web"), { recursive: true });
    mkdirSync(join(dir, "src"), { recursive: true });
    for (const name of [...new Bun.Glob("*").scanSync({ cwd: WEB_ROOT, onlyFiles: true })]) {
      await Bun.write(join(dir, "web", name), Bun.file(join(WEB_ROOT, name)));
    }
    await Bun.write(join(dir, "src", "model.ts"), Bun.file(join(import.meta.dir, "model.ts")));
    const result = await Bun.build({
      entrypoints: [join(dir, "web", "main.ts")],
      target: "browser",
      naming: "[name].[ext]",
      minify: false,
    });
